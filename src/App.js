...
function App() {
  ...
  const handleAddTodo = async () => {
    try {
      await axios.post('http://localhost:8002/api/todos', { task });
      fetchTodos();
      setTask('');
    } catch (error) {
  ...
  const handleUpdateTodo = async (id) => {
    try {
      await axios.put(`http://localhost:8002/api/todos/${id}`, { completed: true });
      fetchTodos();
    } catch (error) {
  ...
  const handleDeleteTodo = async (id) => {
    try {
      await axios.delete(`http://localhost:8002/api/todos/${id}`);
      fetchTodos();
    } catch (error) {
